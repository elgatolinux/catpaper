...
Item {
    ...
    PywalTheme { id: _theme }
    Backend { id: backend; objectName: "backend" }
    ...
    property bool isDownloadingWallpaper: backend.isDownloading
    property string currentDownloadName: backend.downloadingName
    property string backendNotice: backend.notice
    ...
    Connections {
        target: backend
        function onMonitorsReady(names) {
            monitorModel.clear();
            for (var i = 0; i < names.length; i++) {
                monitorModel.append({ "name": names[i], "selected": true });
            }
        }
        function onWallpaperApplied(name) {
            window.targetWallName = name;
        }
        function onLocalCacheReset() {
        ...
        function onThumbsRefreshed() {
            window.refreshModel(localFolderModel, window.thumbDir);
        }
        function onSearchRefreshed() {
            window.refreshModel(searchFolderModel, window.searchDir);
        }
        function onMarkersRefreshed() {
            window.refreshModel(markerModel, window.markerDir);
        }
        function onSrcRefreshed() {
            window.refreshModel(srcModel, "file://" + window.srcDir);
        }
    }

    function refreshModel(model, dir) {
        if (!model) return;
        model.folder = "";
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        backend.applyWallpaper(safeFileName, isVideo, outputs);
    }
    ...
    onIsSearchPausedChanged: {
        backend.setSearchPaused(isSearchPaused)
    }
    ...
    property string currentNotification: {
        if (window.backendNotice !== "") return window.backendNotice;
    ...
    function triggerOnlineSearch() {
        ...
        backend.search(window.searchQuery);
        
        searchInput.focus = false;
        view.forceActiveFocus();
    }

    readonly property string homeDir: backend.homeDir
    readonly property string thumbDir: backend.thumbDir
    readonly property string searchDir: backend.searchDir
    readonly property string markerDir: "file://" + paths.getCacheDir("wallpaper_picker") + "/colors_markers"
    readonly property string srcDir: backend.wallpaperDir
    ...
    readonly property real barCenterOffset: window.s(15) - (window.itemHeight + window.s(30)) / 2 - window.s(16) - window.s(28)
    ...
    Shortcut { sequence: "Escape"; enabled: !window.isApplying; onActivated: { if (window.currentFilter === "Search") { window.currentFilter = "All"; } else { backend.shutdown(); Qt.quit(); } } }
    ...
    ListView {
        ...
        delegate: Item {
            ...
            Item {
                ...
                width: parent ? (parent.width > 0 ? parent.width * (targetWidth / (targetWidth + window.spacing)) : 0) : 0
    ...
    Rectangle {
        ...
        anchors.verticalCenter: parent ? parent.verticalCenter : undefined
        anchors.verticalCenterOffset: window.isReady ? window.barCenterOffset : window.barCenterOffset - window.s(120)
        opacity: window.isReady ? 1.0 : 0.0
        ...
        Row {
            ...
            Rectangle {
                ...
                clip: true
                anchors.verticalCenter: parent ? parent.verticalCenter : undefined
                color: folderMouse.containsMouse ? _theme.surface2 : "transparent"
                border.color: folderMouse.containsMouse ? _theme.text : _theme.surface1
                border.width: 1
                ...
                MouseArea {
                    ...
                    enabled: !window.isApplying
                    cursorShape: Qt.PointingHandCursor
                    onClicked: backend.chooseWallpaperDir()
                }

                Canvas {
                    width: window.s(20)
                    height: window.s(18)
                    anchors.centerIn: folderMouse
                    property string activeColor: folderMouse.containsMouse ? _theme.text : Qt.rgba(_theme.text.r, _theme.text.g, _theme.text.b, 0.7)
                    onActiveColorChanged: requestPaint()
